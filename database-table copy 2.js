...
class SermonDatabase {
  ...
  getSermon(uid) {
    ...
    const sermonWithData = this._stmtGetSermon.all(uid);
  ...
  /**
   * Returns sermon data as a JSON string. Uses a dedicated string cache
   * so the string can be transferred across worker/IPC boundaries without
   * expensive structured-clone of nested objects (string = fast memcpy).
   */
  getSermonJson(uid) {
    this.ensureInitialized();

    if (this.sermonJsonCache.has(uid)) {
      return this.sermonJsonCache.get(uid);
    }

    const sermon = this.getSermon(uid);
    if (!sermon) return null;

    const json = JSON.stringify(sermon);
    this.sermonJsonCache.set(uid, json);
    return json;
  }
