...
const setupDatabaseHandlers = () => {
  ...
  ipcMain.handle('db:searchText', async (event, query, limit, searchMode = 'phrase') => {
    try {
      return await sermonDatabase.searchText(query, limit, searchMode);
    } catch (error) {
...
const createWindow = async () => {
  try {
    mainWindow = new BrowserWindow({
      ...
      height: 1000,
      resizable: true,
      thickFrame: true,              // ensure Windows resize border is present
      frame: true,                   // keep OS frame for resizing
      transparent: false,            // avoid breaking edge hit-testing
      backgroundColor: '#00000000',  // fully transparent content area
      backgroundMaterial: 'acrylic', // Windows acrylic
      visualEffectState: 'active',   // ensure material is applied
    ...
    // Optional: you can keep this for platform-specific tweaks, just ensure it's transparent on Win
    if (process.platform === 'win32') {
      // Try one of these alphas: 0x1A (10%), 0x26 (15%), 0x33 (20%)
      mainWindow.setBackgroundColor('#1A000000'); // 10% black tint over acrylic
    } else if (process.platform === 'darwin') {
      mainWindow.setBackgroundColor('#60000000'); // macOS dark tint (vibrancy is mac-only)
    } else {
      mainWindow.setBackgroundColor('#80000000'); // Linux fallback
    }
