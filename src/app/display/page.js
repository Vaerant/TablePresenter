...
export default function DisplayPage() {
  ...
  const [currentDisplaySettings, setCurrentDisplaySettings] = useState(null);
  ...
  const previousParagraphRef = useRef(null);
  const { selectedParagraph, displaySettings } = useSermonStore();

  // Hide content immediately when paragraph changes
  useEffect(() => {
    if (currentParagraph && 
        JSON.stringify(currentParagraph) !== JSON.stringify(previousParagraphRef.current)) {
  ...
  useEffect(() => {
    ...
    const calculateOptimalFontSize = () => {
      ...
      // Mark calculation as complete and show content immediately
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && window.electronAPI) {
      ...
      const handleParagraphUpdate = (event, data) => {
        console.log('Display received paragraph update via IPC:', data);
        // Update both paragraph and display settings
        if (data.paragraphData && JSON.stringify(data.paragraphData) !== JSON.stringify(currentParagraph)) {
          setCurrentParagraph(data.paragraphData);
        }
        if (data.displaySettings) {
          setCurrentDisplaySettings(data.displaySettings);
        }
      };

      const handleDisplaySettingsUpdate = (event, settings) => {
        console.log('Display received settings update via IPC:', settings);
        setCurrentDisplaySettings(settings);
        // Don't clear the current paragraph - just update settings
      };
      ...
      window.electronAPI.on('display:settingsUpdated', handleDisplaySettingsUpdate);
      ...
      window.electronAPI.paragraph.getCurrentSelection().then((selection) => {
        ...
        if (selection) {
          setCurrentParagraph(selection.paragraphData);
          setCurrentDisplaySettings(selection.displaySettings);
        }
      }).catch(error => {
      ...
      return () => {
        if (window.electronAPI && window.electronAPI.off) {
          ...
          window.electronAPI.off('display:settingsUpdated', handleDisplaySettingsUpdate);
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (message.type === 'paragraph:updated') {
              if (message.data.paragraphData && JSON.stringify(message.data.paragraphData) !== JSON.stringify(currentParagraph)) {
                setCurrentParagraph(message.data.paragraphData);
              }
              if (message.data.displaySettings) {
                setCurrentDisplaySettings(message.data.displaySettings);
              }
            } else if (message.type === 'display:settingsUpdated') {
              console.log('Received display settings update, keeping current paragraph');
              setCurrentDisplaySettings(message.data);
              // Don't clear the current paragraph - just update settings
      ...
      const fetchCurrentParagraph = async () => {
        try {
          ...
          // Handle the new data structure from HTTP API
          if (data) {
            ...
            if (data.paragraphData && JSON.stringify(data.paragraphData) !== JSON.stringify(currentParagraph)) {
              setCurrentParagraph(data.paragraphData);
            } else if (!data.paragraphData && currentParagraph) {
              // Clear paragraph if none selected
              setCurrentParagraph(null);
            }
            
            // Update display settings if provided
            if (data.displaySettings && JSON.stringify(data.displaySettings) !== JSON.stringify(currentDisplaySettings)) {
              setCurrentDisplaySettings(data.displaySettings);
            }
          }
        } catch (error) {
  ...
  useEffect(() => {
    if (selectedParagraph && !currentParagraph) {
      console.log('Syncing paragraph from Zustand store:', selectedParagraph);
      setCurrentParagraph(selectedParagraph);
    }
    if (displaySettings && !currentDisplaySettings) {
      console.log('Syncing display settings from Zustand store:', displaySettings);
      setCurrentDisplaySettings(displaySettings);
    }
  }, [selectedParagraph, currentParagraph, displaySettings, currentDisplaySettings]);
  ...
  if (!shouldShowContent && !shouldShowTitle && !shouldShowDate) {
    return (
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 py-2 text-center fixed top-0 left-1/2 transform -translate-x-1/2 w-full z-10 mt-10">
        {shouldShowTitle && currentParagraph.sermonTitle && (
          <h1 className="text-xl font-bold text-gray-300 mb-1">
            {currentParagraph.sermonTitle}
          </h1>
        )}
        {shouldShowDate && currentParagraph.sermonDate && (
      ...
      <div className="flex-1 flex items-center justify-center px-8 py-2">
        {shouldShowContent && fontSizeCalculated && showContent ? (
